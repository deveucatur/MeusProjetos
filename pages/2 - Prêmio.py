...
WHERE 
	PPE.bonificado_fgkey = (SELECT id_user FROM projeu_users WHERE Matricula = 56126)
    AND
...
if len(premiosuser) > 0:
    ...
    with col2:    
        ...
        with col4:
            ...
            font_TITLE(f'{sum([x[9] for x in premiosuser])}', fonte_Projeto,"'Bebas Neue', sans-serif", 44, 'center')
        with col5:
    ...
    for name_proj in list(set([str(x[4]).strip() for x in premiosuser])):
        ...
        for name_entrg in tarefas_do_project:
            with col1:
                st.text_input('Atividade', name_entrg[5], key=f'atividade{name_proj} - {name_entrg}', label_visibility="collapsed")
            with col2:
                st.text_input('Sprint', name_entrg[1], key=f'sprint{name_proj} - {name_entrg}', label_visibility="collapsed")
            with col3:
                st.text_input('Horas', f'{name_entrg[9]} hrs', key=f'horas{name_proj} - {name_entrg}', label_visibility="collapsed")
            with col4:
                st.text_input('Compl.', complexidade_name(name_entrg[11]), key=f'complex{name_proj} - {name_entrg}', label_visibility="collapsed")
            with col5:
